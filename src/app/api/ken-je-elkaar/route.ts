...
export async function POST(request: NextRequest) {
  try {
    ...
    const { question, answer, type, toelichting, threshold } = await request.json();
    if (!question || !answer) return NextResponse.json({ error: 'Missing question or answer' }, { status: 400 });

    const id = await addKenJeElkaar(question, answer, type || 'number', toelichting, threshold);
...
export async function PATCH(request: NextRequest) {
  try {
    ...
    const { id, question, answer, type, toelichting, threshold } = await request.json();
    if (!id) return NextResponse.json({ error: 'Missing id' }, { status: 400 });

    await updateKenJeElkaar(id, question, answer, type, toelichting, threshold);
